...
- Enforce an administrator-configured HA identity allowlist for console reads and writes. Sidebar visibility alone does not authorize access. Existing installs must add trusted user IDs in Configuration; NATS continues running while the list is empty.
